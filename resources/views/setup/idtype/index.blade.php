...
@section('content')
    <div class="p-6 space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">ID Type Management</h1>
                <p class="text-sm text-gray-500">
                    Manage Employee ID types
                </p>
            </div>
                ...
                <tbody class="divide-y">
                    @forelse($idTypes as $idType)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 w-[150px]">{{ $idType->name }}</td>
                            <td class="px-4 py-3 w-[200px]">{{ $idType->description }}</td>
                            <td class="px-4 py-3 w-[100px] text-xs font-semibold">
                                @php
                                    ...
                                    $status = $statuses[$idType->is_active] ?? [
                            ...
                            <td class="px-4 py-3 w-[50px]">
                                <div class="flex items-center justify-center space-x-2">
                                    <button type="button" title="Edit ID Type {{ $idType->name }}"
                                        data-modal-target="edit-modal" data-modal-toggle="edit-modal"
                                        data-id="{{ $idType->id }}" data-name="{{ $idType->name }}"
                                        data-description="{{ $idType->description }}"
                                        data-status="{{ $idType->is_active }}" onclick="openEditModal(this)"
